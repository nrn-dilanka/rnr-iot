...
const ESP32Manager = () => {
  ...
  const loadNodes = async () => {
    try {
      ...
      console.log('ESP32Manager API response:', response);
      
      // Ensure we always have an array
      const nodeData = response?.data || response || [];
      console.log('ESP32Manager node data:', nodeData);
  ...
  const sendServoCommand = async (values) => {
    ...
    try {
      console.log(`🔧 Sending servo command to ${selectedNode.node_id}: ${values.angle}°`);
      
      // Send real MQTT command through the API
      await nodeAPI.sendServoCommand(selectedNode.node_id, values.angle);
      
      message.success(`✅ Servo command sent wirelessly to ${selectedNode.name}: ${values.angle}°`);
      setServoModalVisible(false);
      form.resetFields();
    } catch (error) {
      console.error('Error sending servo command:', error);
      message.error('❌ Failed to send servo command via WiFi/MQTT');
    }
  };
  ...
  const sendRebootCommand = async (nodeId) => {
    try {
      ...
      // Send real MQTT command through the API
      await nodeAPI.sendRebootCommand(nodeId);
      
      message.success(`✅ Reboot command sent wirelessly to device ${nodeId}`);
    } catch (error) {
      console.error('Error sending reboot command:', error);
      message.error('❌ Failed to send reboot command via WiFi/MQTT');
    }
  };
