...
function Start-Services {
    ...
    try {
        ...
        Write-Host "❌ Cannot check RabbitMQ health" -ForegroundColor Red
    }
...
function Generate-TestData {
    ...
    if (Test-Path "quick_rabbitmq_test.py") {
        Write-Host "Found quick test script, running..." -ForegroundColor Yellow
        
        # Install required packages
        pip install paho-mqtt requests 2>$null
        
        Write-Host "🔄 Running RabbitMQ test and data generation..." -ForegroundColor Green
        python quick_rabbitmq_test.py
    } else {
        Write-Host "❌ quick_rabbitmq_test.py not found" -ForegroundColor Red
    }
}
